...
menu_admin_cache_assert(
    strpos($adminCode, "MENU_CACHE_remove_instance(") === false,
    'Admin code must not invalidate individual cache instances directly'
);
menu_admin_cache_assert(
    strpos($adminCode, 'MENU_invalidateRuntimeCache(true)') !== false,
    'Admin mutations must use centralized runtime cache invalidation'
);
menu_admin_cache_assert(
    strpos($adminCode, 'MENU_invalidateRuntimeCache(false)') !== false,
    'Drag ordering may invalidate cache without rebuilding HTML state'
);
menu_admin_cache_assert(
    strpos($adminCode, 'function MENU_deleteMenu(') === false,
    'Dead whole-menu recursive delete wrapper must stay removed'
);
menu_admin_cache_assert(
    strpos($mutations, 'function MENU_deleteChildElements(') !== false,
    'Subtree deletion must remain available for deleting an element with children'
);
menu_admin_cache_assert(
    strpos($mutations, '" AND menu_id=" . (int) $menu_id') !== false,
