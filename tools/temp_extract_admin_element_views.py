...
module_path = Path('admin_element_views.php')

s = index_path.read_text()
module = module_path.read_text().rstrip() + '\n\n'

include_needle = "require_once $_CONF['path'].'plugins/menu/admin_menu_mutations.php';\n"
include_line = "require_once $_CONF['path'].'plugins/menu/admin_element_views.php';\n"
if include_needle not in s:
    raise SystemExit('admin mutation include not found')
...
for name in names:
    ...
    if comment_start >= 0 and comment_end >= 0 and s[comment_end + 2:pos].strip() == '':
        start = comment_start
